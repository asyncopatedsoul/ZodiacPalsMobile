...
namespace PrefabEvolution
{
	class PEExposedPropertiesEditor
	{
		...
		static public bool CanEditProperties(PEPrefabScript[] targets)
		{
			return !(targets.Length > 1 || targets[0] == null || PrefabUtility.GetPrefabParent(targets[0].gameObject) != targets[0].Prefab);
		}

		void DrawLock(Rect rect)
		{
			if (!(CanEditProperties(this.targets)))
				return;
