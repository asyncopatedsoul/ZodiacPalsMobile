	...
	class PECache : ScriptableObject
	{
		...
		public void CheckPrefab(params string[] paths)
		{
			...
			foreach (var path in prefabsPaths)
			{
				...
				if (i % 100 != 0 || prefabsPaths.Length < 5)
					continue;
		...
		static string GetAssetUserData(string path)
		{
			var meta = getMetaInfo(path);
			var userData = "";
			if (meta != string.Empty)
				userData = getMetaUserData(meta).Replace(":", "_");
			else
			{
				var importer = AssetImporter.GetAtPath(path);
				if (importer == null)
		...
		static bool canPrefabContainsNestedInstances(string path)
		{
			try
			{
				var userData = GetAssetUserData(path);

				if (userData != HaveConst && userData != DontHaveConst)
					return true;
				return userData == HaveConst;
			}
			catch (Exception)
			{
				return true;
			...
			void OnPostprocessModel(GameObject gameObject)
			{
				if (PEPrefs.AutoModels)
				{
					...
					EditorApplication.delayCall += () =>
					{
						...
						Instance.CheckPrefab(this.assetPath);
					};
				}
			}
		}
