	...
	static internal class PEUtils
	{
		...
		private static void OnPropertyContextMenu(GenericMenu menu, SerializedProperty property)
		{
			...
			menu.AddItem(new GUIContent("Expose Property"), exposed, () =>
			{
				if (!exposed)
				{
					prefabScript.Properties.Add(new ExposedProperty() { Label = property.displayName, PropertyPath = property.propertyPath, Target = property.serializedObject.targetObject });
		...
		static internal IEnumerable<PEPrefabScript> GetNestedInstances(GameObject gameObject)
		{
			...
			foreach (var instance in instances)
			{
				...
				if (parentInstance == rootInstance)
				{
					...
					{
						var peComponent = rootInstance.ParentPrefab.GetComponent<PEPrefabScript>();
						if (peComponent)
						{
							if (peComponent.Links[parentInstance.Links[instance]] == null)
								yield return instance;
						}
						else
						{
							Debug.LogErrorFormat("Missing PrefabScript at '{0}'" + AssetDatabase.GetAssetPath(rootInstance.ParentPrefab), rootInstance.ParentPrefab);
						}
					}
				}
				else
		...
		static public GameObject FindRootPrefab(GameObject obj)
		{
			if (obj == null)
				return null;

			return PrefabUtility.FindPrefabRoot(obj) ?? PrefabUtility.FindValidUploadPrefabInstanceRoot(obj);
		}
		...
		static internal bool PropertyFilter(SerializedProperty property)
		{
			...
			return propertyPath.StartsWith("m_Prefab") ||
			propertyPath.StartsWith("m_FileID") ||
			propertyPath.StartsWith("m_PathID") ||
			propertyPath.StartsWith("m_ObjectHideFlags") ||
			propertyPath.StartsWith("m_Children") ||
			propertyPath.StartsWith("m_Father") ||
			propertyPath.StartsWith("m_GameObject") ||
			propertyPath.StartsWith("m_Component");
		}
